...
- B. Rank 4 implies the model has not yet passed the safety threshold required for deployment, because Arena's pairwise rating system penalises refusal-heavy responses by construction and a top-tier rank under that rule is itself a safety red flag.
...
- D. The headline rank is one coordinate; without a CI on the rank, the cluster size, and composition with capability and safety axes, it is a marketing artefact. Arena rewards user-preferred style, not correctness or safety.
