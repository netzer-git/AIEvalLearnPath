...
Two prior lessons are load-bearing today. **[D-1](/lesson/1)'s pipeline framing** — that an evaluation is a (dataset, scoring rule, reporting convention) triple — is what lets us locate the claimed-vs-effective gap as a property of the *reporting convention*: spec sheets advertise an architectural cap, but RULER's *scoring rule* is "highest $L$ at which mean accuracy $\geq \tau$", and the two answer different questions about the same model. **[D-5](/lesson/5)'s statistical-hygiene machinery** — confidence intervals and the "is a delta resolvable?" framing — is the load-bearing toolkit for reading any RULER score: the effective-context-length metric is itself a CI-aware aggregation (a *threshold* over a noisy curve, not a point estimate), and the same per-item SE math from [D-5](/lesson/5) is what tells you whether two adjacent length-bins are distinguishable. Today's lesson is the first capability-axis lesson where the input itself, not just the scoring rule, becomes the object of measurement.
