...
- A. 80 expert-authored multi-turn questions across 8 categories, judged by GPT-4 in pointwise mode.
- B. ~1,024 real-user tasks drawn from the WildChat conversation corpus, with two scoring metrics (WB-Score pointwise, WB-Reward pairwise vs. three baselines), per-item checklists generated by GPT-4-Turbo + Claude-3-Opus, and an explicit K-character length-penalty mitigation that converts marginal length-driven wins to ties.
- C. 500 prompts mined from Chatbot Arena via BERTopic clustering, with Bradley-Terry aggregation over GPT-4-Turbo pairwise judgments.
- D. ~12,000 questions across 14 disciplines with 10 answer choices each.

**Q2.** Wang et al. (2023), *Large Language Models are not Fair Evaluators*, demonstrated that:

- A. ChatGPT could be made to prefer Vicuna-13B's responses on 66 of 80 queries simply by ordering Vicuna's response first — i.e., the judgment changes when the pair is swapped, exposing systematic position bias in pairwise LLM judging.
- B. GPT-4 reaches over 80% agreement with humans on MT-Bench.
- C. RLAIF-trained models grow longer with each release.
- D. Self-recognition correlates linearly with self-preference bias strength.
...
- A. Trust both numbers as long as the harness is unchanged.
- B. Trust the first (GPT-4-Turbo judge) more than the second (own-model judge), because Panickssery et al. 2024's self-preference bias result implies a systematic upward bias in the second number; the second number is consistent with self-preference inflation rather than genuine improvement, and a cross-family judge panel would be the standard discriminator.
- C. Trust the second number more because the model knows itself best.
- D. Conclude both numbers are uninterpretable and ignore them.

**Q4.** Which of the following is the **best** statement of D22's Goodhart mechanism, distinguishing it from D6/D15/D17?

- A. The judge's training data is contaminated with the test set, so the score measures memorization (this is D6's mechanism).
- B. The benchmark's reference set treats refusal-shaped strings as truthful, so the score selects for refusal rather than truth-tracking (this is D15's mechanism).
- C. The model conditions on whether the input looks like an evaluation, so the eval-distribution sample is not deployment-representative (this is D17's mechanism).
- D. The judge's systematic biases (verbosity, position, self-preference, bandwagon) become the optimization gradient when models are RL-tuned against judge-derived signals; the measurement *instrument itself* is the target. The leak is in the grader, not the test items, the rubric, or the input distribution — and the defense correspondingly operates on the judge model (cross-family panels, length controls, swap-rate audits) rather than on the test set or the rubric.

**Q5.** WildBench's WB-Reward pipeline includes a length-penalty parameter $K$ measured in characters. What does it do, and why?

- A. It penalizes any response longer than $K$ characters by subtracting $K$ from the score.
- B. When a "Slightly Better" / "Slightly Worse" outcome would have favored the longer response over the shorter by more than $K$ characters, the outcome is **converted to Tie** — directly canceling the small wins that are most likely to be driven by verbosity bias rather than substantive quality. WildBench reports both the with-penalty and without-penalty numbers, so the verbosity component is legible.
- C. It is a regularization term in a downstream RL training loop, not part of the eval scoring.
- D. It only applies to responses generated by the judge model itself.
...
- A. (a) is the headline because it's the highest-precision number; (b) and (c) are noisier.
- B. The MT-Bench number (a) is the *least* trustworthy: same-family single-judge with no position-balancing is exposed to all four canonical biases at once. The WildBench gap between with-penalty and without-penalty (+12 vs. +28) is the most informative single signal — it's the verbosity-bias component made legible. Arena-Hard-Auto (b) provides the cross-check against human-preference rankings. The right report cites (b) and (c) with the gap, and treats (a) as a methodologically suspect legacy number.
- C. (b) is the only number worth citing; (a) and (c) should be discarded.
- D. All three numbers are equivalent under length normalization.
